            ...
            render """
                {
                  "explore": {
                    ...
                    "work": {
                      "name": {
                        "count": ${dataAsJson.findAll({ it?.occupation?.type == 'work' && it?.occupation?.name != '✔ Эта страница официально подтверждена' }).collect({ it?.occupation?.name }).unique().size()},
                        "distribution": ${dataAsJson.findAll({ it?.occupation?.type == 'work' && it?.occupation?.name != '✔ Эта страница официально подтверждена' }).collect({ it?.occupation?.name }).groupBy({ it }).sort({ -it.value.size() }).collect({ key, value -> [ 'name': key, 'count': value.size() ] }).take(10)}
                      }
                    },
                    "career": {
                      "size": {
